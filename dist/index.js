...
app.post("/rooms", (req, res) => {
    const roomId = (0, crypto_1.randomBytes)(4).toString("hex");
...
wss.on("connection", (ws, req) => {
    ...
    const socketId = (0, crypto_1.randomBytes)(4).toString("hex");
    const color = COLORS[room.size % COLORS.length];
    const user = { ws, name, socketId, color, emoji };
    ...
    ws.on("message", (data) => {
        ...
        if (msg.to) {
            ...
            if (target?.ws.readyState === ws_1.WebSocket.OPEN) {
...
function broadcast(room, excludeId, msg) {
    ...
    room.forEach((user, id) => {
        if (id !== excludeId && user.ws.readyState === ws_1.WebSocket.OPEN) {
            user.ws.send(data);
        }
    });
}
